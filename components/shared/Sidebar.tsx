        ...
        {modules.items.map((sidebarItem,index)=>{
          return(
            <React.Fragment key={index}>
              <h6 className='text-zinc-50 font-semibold'>{sidebarItem.heading}</h6>
              <ul className='flex flex-col w-full gap-1'>
                {sidebarItem.elements.filter((sidebarSubItem)=>sidebarSubItem.label.toLocaleLowerCase().includes(filter.toLocaleLowerCase())).map((sidebarSubItem)=>{
                  return(
