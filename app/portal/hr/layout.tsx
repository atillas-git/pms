...
const HrLayout = async ({children}:{children:React.ReactNode}) => {
    ...
    const isAuthenticated = session.permissions
        .filter((permission:string)=>["hr.employee","hr.manager","administrator"].includes(permission)).length > 0
    

  return (
    <div>
        {isAuthenticated ? children : <InvalidPermissions/>}
    </div>
  )
}
